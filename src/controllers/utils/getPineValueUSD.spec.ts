...
describe('utils', () => {
  ...
  beforeEach(async () => {
    const { getRequest } = await esmock('./getPineValueUSD', {
      './getRequest': (getRequestStub = async () => ({ amount: new BigNumber(1000), currency: 'USD' })),
    })
  })
  ...
  //   it('should fetch PINE value using coingecko', async () => {
  //     const fakeData = { pine: { usd: '600' } }
  //     getRequestStub.resolves(fakeData)

  //     const { useCoingecko } = await esmock('./getPineValueUSD', { './getRequest': getRequestStub })
  //     const resultFunction = await useCoingecko(3)
  //     const result = await resultFunction()
  //     const expectedValue = new BigNumber(1800)
  ...
  //     getRequestStub.rejects(new Error('API Error'))
