...
export default async function getLoanTerms({ blockchain, collectionId, nftId }: Params): Promise<LoanTerms> {
  ...
  try {
    ...
    case 'ethereum': {
      ...
      if (ETHLimits[pool.address] && loanTerms.options.some(option => utilization.amount.plus(option.maxBorrow?.amount ?? new BigNumber(0)).gt(new BigNumber(ETHLimits[pool.address])))) throw fault('ERR_POOL_OVER_LENDER_DEFINED_UTILIZATION')
