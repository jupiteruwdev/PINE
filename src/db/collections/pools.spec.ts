...
describe('db/collections', () => {
  ...
  it('can find all pools on Ethereum Mainnet only', async () => {
    const res = await pools.findAllPools({ blockchainFilter: { ethereum: EthereumNetwork.MAIN } })
    assert.isTrue(res.length > 0)
  ...
  it('can find all pools on Ethereum Rinkeby only', async () => {
    const res = await pools.findAllPools({ blockchainFilter: { ethereum: EthereumNetwork.RINKEBY } })
    assert.isArray(res)
    assert.isTrue(res.length > 0)
