...
describe('db/collections', () => {
  ...
  it('can find all collections on Ethereum Mainnet only', async () => {
    const res = await collections.findAll({ blockchains: { ethereum: EthereumNetwork.MAIN } })
    assert.isArray(res)
    assert.isTrue(res.length > 0)
  ...
  it('can find all collections on Ethereum Rinkeby only', async () => {
    const res = await collections.findAll({ blockchains: { ethereum: EthereumNetwork.RINKEBY } })
    assert.isArray(res)
    assert.isTrue(res.length > 0)
